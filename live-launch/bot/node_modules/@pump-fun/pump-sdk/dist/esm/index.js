...
function newBondingCurve(global, quoteMint = PublicKey2.default) {
  return {
    virtualTokenReserves: global.initialVirtualTokenReserves,
    virtualQuoteReserves: quoteMint ? quoteMint.equals(NATIVE_MINT) || quoteMint.equals(PublicKey2.default) ? global.initialVirtualSolReserves : global.initialVirtualQuoteReserves : global.initialVirtualSolReserves,
    realTokenReserves: global.initialRealTokenReserves,
    realQuoteReserves: new BN2(0),
    tokenTotalSupply: global.tokenTotalSupply,
    complete: false,
    creator: PublicKey2.default,
    isMayhemMode: global.mayhemModeEnabled,
    isCashbackCoin: false,
    quoteMint: PublicKey2.default
  };
}
function getBuySolAmountFromTokenAmountQuote({
  ...
  return minAmount.mul(virtualQuoteReserves).div(virtualTokenReserves.sub(minAmount)).add(new BN2(1));
}
...
function getStaticRandomFeeRecipientForBuyback() {
  ...
  return new PublicKey2(CURRENT_FEE_RECIPIENTS_FOR_BUYBACK[randomIndex]);
}
...
var PumpSdk = class {
  ...
  getCoinCreatorVaultQuoteMint(coinCreator, coinCreatorVaultAta) {
    ...
    const candidates = [
      {
        mint: new PublicKey4("So11111111111111111111111111111111111111112"),
        tokenProgram: TOKEN_PROGRAM_ID2
      },
      {
        mint: new PublicKey4("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"),
        tokenProgram: TOKEN_PROGRAM_ID2
      },
      {
        mint: new PublicKey4("4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU"),
        tokenProgram: TOKEN_PROGRAM_ID2
      }
    ];
  ...
  async createV2Instruction({
    ...
    if (quoteMint && !isLegacyQuoteMint(quoteMint)) {
      return await builder.remainingAccounts([
        ...
        { pubkey: TOKEN_PROGRAM_ID2, isWritable: false, isSigner: false }
      ]).instruction();
    }
  ...
  async createV2AndBuyV2Instructions({
    ...
    const buyQuoteMint = quoteMint && !isLegacyQuoteMint(quoteMint) ? quoteMint : NATIVE_MINT3;
    return [
      await this.createV2Instruction({
        mint,
        name,
  ...
  async buyV2Instruction({
    ...
    return await this.getBuyV2InstructionInternal({
      ...
      quoteAmount: quoteAmount.add(
        quoteAmount.mul(new BN5(Math.floor(slippage * 10))).div(new BN5(1e3))
      ),
      tokenProgram,
      quoteMint,
      quoteTokenProgram
...
var isLegacyQuoteMint = (quoteMint) => quoteMint.equals(NATIVE_MINT4) || quoteMint.equals(PublicKey5.default);
...
var DONATION_RELAY_PROGRAM_ID_MAINNET = new PublicKey5(
  "RLAYHr9TRFcKB2ubYQhspcnXiaGpaVzNQvHytt47RZu"
);
var DONATION_RELAY_PROGRAM_ID_DEVNET = new PublicKey5(
  "DRLYxueWz6iymdsaRCER6iv6v9zL7gFWANwDL2V5VUx1"
);
var donationRelayPda = (seeds, donationRelayProgramId) => {
  return PublicKey5.findProgramAddressSync(seeds, donationRelayProgramId)[0];
};
