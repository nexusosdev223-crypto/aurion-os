...
export const MAX_DRAWDOWN: number      = 0.20;   // 20 % — absolute circuit brake
...
export async function getEconomicSnapshot(): Promise<EconomicSnapshot> {
  ...
  const txs      = rows ?? [];
  let runningCap  = 1_000_000;               // $\nseeded capital baseline
  ...
  const drawdownPct    = peakCapital > 0
    ? (peakCapital - currentCapital) / peakCapital
    : 0;
...
export async function auditGateDecision(params: {
  ...
  const { error } = await supabase.from('aurion_ledger').insert([{
    ...
    execution_result:  params.executionResult ?? (params.gatePassed ? 'EXECUTED' : 'BLOCKED'),
